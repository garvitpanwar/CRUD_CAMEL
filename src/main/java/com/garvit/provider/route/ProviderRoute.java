...
//    private final Validator validator = Validation.buildDefaultValidatorFactory().getValidator();
    ...
    public void configure() {
        ...
        onException(ConstraintViolationException.class)
                ...
                .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(400))
                .setBody(exchange ->
                        new ResponseMessage("Partner ID or Name already exists (duplicate entry)")
                )
                .marshal().json(JsonLibrary.Jackson);
        ...
        onException(BeanValidationException.class)
                ...
                .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(400))
                .setBody(exchange -> Map.of("errors", exchange.getProperty(Exchange.EXCEPTION_CAUGHT, Exception.class).getMessage()))
                .marshal().json(JsonLibrary.Jackson);
        ...
        onException(Exception.class)
                ...
                .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(500))
                .setBody(exchange -> new ResponseMessage("${exception.message}"))
                .marshal().json(JsonLibrary.Jackson);
        ...
        from("direct:addProvider")
                .routeId("add-provider-route")
                .log("[AddProvider] - Received request: ${body}")
                .to("bean-validator://providerValidator")
                .log("[AddProvider] - Validation passed")
                .bean(providerService, "saveProvider(${body})")
                .log("[AddProvider] - Provider saved with ID: ${body.id}")
                .setBody(exchange -> new ResponseMessage("Provider added successfully"))
                .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(201))
        ...
        from("direct:getAllProviders")
                .routeId("get-all-providers-route")
                .log("[GetAllProviders] - Fetching providers")
                .bean(providerService, "getAllProviders")
                .choice()
                .when(body().isNull())
                ...
                .setBody(exchange -> new ResponseMessage("No providers found"))
                .otherwise()
                .log("[GetAllProviders] - Found ${body.size} providers")
                .end();

        // Get Provider by ID
        from("direct:getProviderById")
                .routeId("get-provider-by-id-route")
                .log("[GetById] - Fetching provider with ID: ${header.id}")
                .bean(providerService, "getProviderById(${header.id})")
                .choice()
                .when(body().isNull())
                .log("⚠ No provider found for ID ${header.id}")
                .setHeader(Exchange.HTTP_RESPONSE_CODE, constant(404))
                .setBody(exchange -> new ResponseMessage("Provider not found"))
                .otherwise()
                .log("[GetById] - Provider found: ${body}")
                .end();
        ...
        from("direct:deleteProvider")
                .routeId("delete-provider-route")
                .log("Deleting provider with ID: ${header.id}")
                .bean(providerService, "deleteProvider(${header.id})")
                .choice()
                .when(body().isEqualTo(true))
                .log("Deleted provider with ID: ${header.id}")
                .setBody(exchange -> new ResponseMessage("Provider deleted successfully"))
                .otherwise()
